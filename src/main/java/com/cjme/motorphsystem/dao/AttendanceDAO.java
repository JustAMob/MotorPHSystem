...
public class AttendanceDAO {
    ...
    public List<Attendance> getTimeLogsByDateRange(Date startDate, Date endDate) throws SQLException {
    List<Attendance> list = new ArrayList<>();
    String sql = "SELECT * FROM attendance WHERE attendance_date BETWEEN ? AND ?";
    try (PreparedStatement stmt = connection.prepareStatement(sql)) {
        stmt.setDate(1, startDate);
    ...
    public List<Attendance> getTimeLogsByEmployeeAndDateRange(int employeeID, Date startDate, Date endDate) throws SQLException {
    List<Attendance> list = new ArrayList<>();
    ...
    String sql = "UPDATE attendance SET employee_id = ?, attendance_date = ?, time_in = ?, time_out = ? WHERE attendance_id = ?";
    try (PreparedStatement stmt = connection.prepareStatement(sql)) {
        stmt.setInt(1, attendance.getEmployeeID());
        stmt.setDate(2, attendance.getLogDate());
        stmt.setTime(3, attendance.getTimeIn());
        stmt.setTime(4, attendance.getTimeOut());
        stmt.setInt(5, attendance.getLogID()); // Use the ID for WHERE clause
        return stmt.executeUpdate() > 0;
    }
    }
    ...
    public List<Attendance> getAllTimeLogs() throws SQLException {
    List<Attendance> list = new ArrayList<>();
